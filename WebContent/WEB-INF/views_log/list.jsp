...
<!actionCTYPE html><html><head><meta charset="UTF-8">
   ...
   <c:forEach var="board" items="${list }">
      ...
      <c:if test="${board.del=='n' }">
         ...
         <c:if test="${board.re_level > 0 }">      
            ...
            <a href="content.action?num=${board.num}&pageNum=${currentPage}">
                  ${board.subject}</a> 
...
<%--       <button onclick="location.href='list.action?pageNum=${startPage-1}'">[이전]</button> --%>
<%--    </c:if> --%>
<%--    <c:forEach var="i" begin="${startPage}" end="${endPage}"> --%>
<%--       <c:if test="${i==currentPage }"> --%>
<%--             <button onclick="location.href='list.action?pageNum=${i}'" --%>
<!--                disabled="disabled"> -->
<%--             [${i}]</button> --%>
<%--       </c:if>    --%>
<%--       <c:if test="${i!=currentPage }">       --%>
...
<%--       <button onclick="location.href='list.action?pageNum=${endPage+1}'">[다음]</button> --%>
<%--    </c:if> --%>

<hr/>
      <a class="btn btn-primary float-left">글목록</a>
      <a href="insertForm.action?pageNum=1" class="btn btn-primary float-right">글쓰기</a>
